import OctagonBase
import SmallGapBernsteinStandalone
...
open Real
open OctagonBase
open SmallGapBernsteinStandalone
...
lemma square_le_of_nonneg
  ...
  have hp := mul_nonneg (sub_nonneg.mpr hxy) (add_nonneg hy hx)
  nlinarith
...
lemma surrogate_product_le_true
  ...
  exact le_trans (mul_le_mul_of_nonneg_right hs hsum0)
    (mul_le_mul_of_nonneg_left hsum hsin0)
...
lemma rationalGap_le_actual
  ...
  have hprod0 : 0 ≤ lowerSin u * (lowerCos u + lowerSin u) :=
    mul_nonneg (lowerSin_nonneg u hu hub)
...
theorem reserve_low
    (u : ℝ) (hu : 0 ≤ u) (hub : u ≤ (1 : ℝ) / 8) :
    smallGapReserve * (1 - 4 * u) ≤ smallPhaseDefect u := by
...
lemma product_ge_four_u
  ...
  have hz0 : 0 ≤ z := by dsimp [z, x]; nlinarith [Real.pi_pos]
  have hzq : z ≤ Real.pi / 4 := by dsimp [z, x]; nlinarith [Real.pi_pos]
...
lemma defect_minus_cap_line (u : ℝ) :
  ...
  have hrel : phaseSlope = 4 * cap + 4 * d := by
    dsimp [phaseSlope]
    rw [cap_eq]
    dsimp [d]
    nlinarith [s_sq]
