...
lemma ordinary_disjoint_collision :
  ...
  intro o ho hc
  simp only [ordinaryOwners, Finset.mem_filter, Finset.mem_univ,
    true_and] at ho
  simp only [collisionOwners, Finset.mem_filter, Finset.mem_univ,
    true_and] at hc
  omega
...
theorem ordinary_edge_injective :
  ...
  have haCount : fiberCount S (S.edge a.1) = 1 := by
    simpa [ordinaryOwners] using a.2
  have hbCount : fiberCount S (S.edge b.1) = 1 := by
    simpa [ordinaryOwners] using b.2
  have hcard : (fiber S (S.edge a.1)).card = 1 := by
    simpa [fiberCount] using haCount
...
theorem collision_edge_has_two
    ...
    (fiber S e).card = 2 := by
  simpa [collisionEdges, fiberCount] using he
